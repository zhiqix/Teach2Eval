...
class PromptTeacherStrategy(ABC):
    ...
    def format_prompt(self, whole_question_wo_options, conversation_teacher, turn):
        # 使用自定义的模板
        if turn == 1:
            #第一次judge，不用反思
...
class PromptTeacherContext:
    ...
    def format_prompt(self, whole_question_wo_options, conversation_teacher, turn):
        return self._strategy.format_prompt(whole_question_wo_options, conversation_teacher, turn)
